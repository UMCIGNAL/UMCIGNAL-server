...
import { insertCodeService, myRerollService } from "../serialCode.service/serialCode.service";
...
export const myRerollController = async (
    ...
    try {
        ...
        if(check === false) {
            ...
            const result = await myRerollService(user_id);

            return res.status(202).json({ result, message : '현재 리롤 횟수입니다.' });
        } 
